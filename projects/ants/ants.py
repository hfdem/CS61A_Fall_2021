...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        discover_place, position = self.place, 0
        while not discover_place.is_hive:
            if self.min_range <= position <= self.max_range and discover_place.bees:
                return random_bee(discover_place.bees)
            discover_place, position = discover_place.entrance, position + 1
        # END Problem 3 and 4
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8
        assert self.is_container and not ant.is_container
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slowed_length:
            self.slowed_length -= 1
            if gamestate.time % 2:
                # odd turn
                destination = self.place
        if self.scared_length and gamestate.time % 2 == 0:
            # even turn
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        if not self.scared:
            self.scared_length, self.scared = length, True
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            ...
            distance, place = distance + 1, place.entrance
        return insect_dict
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(2 - 0.0625 * self.insects_shot - 0.25 * distance, 0)
        # END Problem Optional 2
