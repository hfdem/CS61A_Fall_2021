...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    sum_rolls, sow_sad = 0, False
    while num_rolls > 0:
        num_dice, num_rolls = dice(), num_rolls - 1
        sum_rolls = sum_rolls + num_dice
        if num_dice == 1:
            sow_sad = True
    return sum_rolls if sow_sad == False else 1
    # END PROBLEM 1
...
def digits_num(digits, num):
    ...
    2
    >>> digits_num(10, 1234567890)
    0
    '''
    assert type(num) == int and type(digits) == int, 'Digits and number must be an integer.'
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        who, score0 = 1 - who, play_player(strategy0, score0, score1, dice, goal)
        if init_comment == False:
            comment = say(score0, score1)
            init_comment = True
        else:
            comment = comment(score0, score1)
        if score0 < goal:
            who, score1 = 1 - who, play_player(strategy1, score1, score0, dice, goal)
            comment = comment(score0, score1)
        else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    average, score, num, highest_score, highest_num = make_averaged(roll_dice, trials_count), 0, 1, 0, 1
    while num <= 10:
        score = average(num, dice)
        if score > highest_score:
            highest_score, highest_num = score, num
        num += 1
    return highest_num
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    '''
    if score > GOAL_SCORE - 20:
        return hog_pile_strategy(score, opponent_score, cutoff=16, num_rolls=5)
    elif score > GOAL_SCORE - 16:
        return hog_pile_strategy(score, opponent_score, cutoff=12, num_rolls=4)
    elif score > GOAL_SCORE - 12:
        return hog_pile_strategy(score, opponent_score, cutoff=GOAL_SCORE - score, num_rolls=3)
    elif score > GOAL_SCORE - 8:
        return hog_pile_strategy(score, opponent_score, cutoff=GOAL_SCORE - score, num_rolls=2)
    elif score > GOAL_SCORE - 5:
